...
1. **Изоляция Рендера от Логики (Front/Back Buffer):**
   - PrepareFrame строит данные и команды очередного кадрового слота только из опубликованного m_PrimaryNodes и стабильных ресурсов. RenderFrame отправляет команды предыдущего слота, не читая сценовое дерево. Экстракция и Draw завершаются в этапе 27.
