...
1. **Изоляция Рендера от Логики (Front/Back Buffer):**
   - PrepareFrame строит данные и команды очередного кадрового слота только из опубликованного m_PrimaryNodes и стабильных ресурсов. RenderFrame отправляет команды предыдущего слота, не читая сценовое дерево. Экстракция и Draw ещё завершаются в этапе 20.
...
2. **Мутации во время обхода скриптов:**
   ...
   - В этапе 19 до разрешения удаления/смены родителя из callback определить безопасную точку и способ применения операций. Конкретный механизм пока не выдаётся за реализованный.
