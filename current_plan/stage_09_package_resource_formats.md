...
- **Статус:** Выполнено по генплану в части CPU-форматов, импорта и чтения MeshData. Этап 25 сначала подготавливает через CPU-контур стабильные vertex/index pointers/spans, а GPU-меш и вершинные/индексные буферы реализуются его будущим продолжением.
...
5. **Разделение ответственности между `core` и `assets_builder_dll`:**
   - **`core` (Рантайм-ядро):**
    ...
    - **`assets_builder_dll` (Тулчейн / Упаковщик / Редактор):**
      - Импорт ресурсов в `data.dat` организован через `AssetImporterRegistry` и `AssetImportPipeline`: для каждого поддерживаемого расширения регистрируется свой `IAssetImporter` (например, `MeshImporter` для `.obj`).
...
6. **Мета-файлы с GUID (`<filename>.meta`):**
    - У файла `cube_00.obj` есть парный `cube_00.obj.meta`:
      ...
      ```
    - `AssetImportPipeline` и `ProjectIdentityValidator` считывают и валидируют обязательный `guid`. Тип ресурса (`Mesh`) однозначно определяется по расширению `.obj` через `AssetImporterRegistry`. Поле `type` является обязательным только для скриптов (`.h.meta`).
...
4. **Парсер `ObjImporter` (C++):**
   - Читает `.obj` $\to$ дедуплицирует в `Vertex3D` (64 байта в текущем C++-формате, CW порядок обхода, опорная точка в начале координат $(0,0,0)$) $\to$ `UInt16` индексы $\to$ `MeshData` $\to$ бинарный блок данных. Stride берётся из реального Vertex3D/формата MeshData; GPU input layout этапов 17–18 обязан ему соответствовать.
