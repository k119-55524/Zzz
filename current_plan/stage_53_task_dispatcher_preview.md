# Этап 53: Диспетчер задач с аппаратной адаптацией ядер (TaskDispatcher)

## 1. Контекст, статус и цель

- **Номер этапа:** 53 (Уровень 6: Apple/mobile и полировка).
- **Статус:** ⏳ В процессе
- **Цель:**
...
- **Отношение к другим этапам:** Этапы 37 (многопоточный culling) и 47 (асинхронная компиляция шейдеров) при их реализации будут использовать готовый централизованный `TaskDispatcher`.
