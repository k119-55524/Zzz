...
> [!IMPORTANT]
> **Текущий активный пункт:** `Пункт 27. Компиляция, рефлексия и упаковка шейдеров (Shader Compilation & Packing)`
>
> **Статус:** 🚀 В работе
> **Список открытых сквозных задач / технического долга:** [`docs/ARCHITECTURE.md`](../docs/ARCHITECTURE.md) §4  
> **Текущая подзадача:** Монтирование трёхсекционного `data.dat` и внешних `N.dat` в `DataAssetsManager` (`ReadOnlyFile` по `eDataDatType`, O(1) доступ по типу); чтение `MeshMetadata` из `PackageEntry`; zero-allocation `MeshPayloadView` над layout `MeshData`; запрос через `CpuResourceManager` с гарантированным lifetime. Далее — GPUBuffer, staging-copy, submit/fence и настоящий GpuMesh.
> 
---
...
| **25** | Библиотека обработки и сжатия текстур (`texture_processor_lib`) | ✅ Выполнено | [`stage_25_texture_processor_library.md`](stage_25_texture_processor_library.md) | Выделенная статическая библиотека для декодирования (PNG, JPG, TGA, BMP, HDR через `stb_image`), валидации кратности 2, генерации mipmap-уровней (`stb_image_resize2` sRGB) и компрессии в GPU-форматы (BC7/BC5/BC4 через DirectXTex для Desktop, ASTC для Mobile). Формирование бинарного блоба с `TextureBlobHeader` (64 байта). |
| **26** | Трёхсекционный TOC data.dat, внешние пакеты и сборщик PackagePacker | ✅ Выполнено | [`stage_26_multipak_toc_and_package_packer.md`](stage_26_multipak_toc_and_package_packer.md) | Разрабатываемый `data.dat` 1.0.0 с PackManifestTable, InlinePackageEntryTable и ExternalPackageEntryTable; имя внешнего пака выводится из `eDataDatType` (`0.dat` для Texture2D); builder-side проверка формата; recoverable directory swap без предварительного удаления рабочего `assets/` |
| **27** | Компиляция, рефлексия и упаковка шейдеров | 🚀 В работе | [`stage_27_shader_compilation_and_packing.md`](stage_27_shader_compilation_and_packing.md) | Компиляция HLSL в DXIL (DX12) и SPIR-V (Vulkan) через DXC; извлечение Shader Reflection (семантики входов, CBV, SRV, Samplers); вычисление requiredStreamsMask для мешей; ShaderBlobHeader и бинарный блоб ShaderData; ShaderImporter в assets_builder_dll и упаковка в data.dat под eDataDatType::Shader |
| **28** | Архитектура хранения меша, планирование стримов и подготовка к GPU upload | ⏳ Не начато | [`stage_28_mesh_multi_stream_and_gpu_upload.md`](stage_28_mesh_multi_stream_and_gpu_upload.md) | Multi-stream хранение меша (базовый Stream 0 48b + опциональные Skinning/Colors/UV1); сборщик PackPlanner накапливает требования материалов (requiredBuffersMask |= ...); упаковка мешей в data.dat без мусора; монтирование трёхсекционного data.dat и 0.dat в DataAssetsManager; чтение напрямую в staging-буфер GPU без дублирования в RAM |
| **29** | Базовые Shader и Material | ⏳ Не начато | — | Создание MaterialData, привязка к скомпилированным шейдерам этапа 27, текстурам Texture2D и константным параметрам; связка слотов материалов с сабмешами |
| **30** | Минимальный сквозной рендер куба | ⏳ Не начато | — | Базовая Camera, View/Projection и aspect при resize; обход DefaultSpatialStorage без BVH; игровой поток после update публикует неизменяемый набор команд кадра N, render-поток исполняет только набор N-1 без чтения живой сцены; DrawIndexed на DX12/Vulkan и раздельные CPU/GPU-барьеры |
| **31** | КП-1 / КП-2: вращающийся текстурированный куб на Windows | ⏳ Не начато | — | Проверка всей цепочки ассет -> пакет/файл -> ресурс -> скрипт -> Draw/Present под DX12 и Vulkan. Resize, minimize/restore, смена сцены и закрытие при загрузке, ошибка обязательного ресурса, отсутствие преждевременного GPU-release; закрытие применимых проверок этапа 10 |

### Уровень 3: ZzzGUI-MVP и первый перенос

Этапы выполняются строго последовательно от **32** до **40**. Расширенные 3D-возможности вынесены в Уровень 4 и не блокируют GUI-MVP.

| № | Этап разработки | Статус | Файл этапа | Краткое описание |
|---|---|---|---|---|
| **32** | Зона рендеринга и DPI | ⏳ Не начато | — | Расширение базового viewport из этапа 30: Scissor, логические единицы DIP/Canvas Units, DPI/размер окна и Safe Area; обновление WindowCommon::OnDpiChanged (TODO 11) |
| **33** | Дерево и разметка ZzzGUI (Measure/Arrange) | ⏳ Не начато | — | gui_lib, размеры и координаты элементов, базовые контейнеры StackPanel/Grid/Canvas/Border, clipping и основа ScrollViewer; геометрия для hit-testing |
| **34** | 2D GPU GUI Batcher | ⏳ Не начато | — | GuiVertex2D, динамические буферы, текстуры и Scissor Rects; вывод результатов layout, lifetime кадровых данных по готовому контракту GAPI |
| **35** | Рендеринг текста | ⏳ Не начато | — | Выбор технологии в момент реализации; измерение текста для layout, глифы и интеграция в batcher, основа TextBlock/TextBox |
| **36** | Ввод и маршрутизация по слоям | ⏳ Не начато | — | Hit-testing по результатам layout, focus/capture, мышь/клавиатура; интерфейсы touch/gamepad с проверкой на соответствующих платформах; поглощение событий GUI и проброс в 3D |
| **37** | Базовые контролы | ⏳ Не начато | — | Button, TextBlock, Image, Slider, ProgressBar, CheckBox, TextBox с фокусом, кареткой и вводом; доведение ScrollViewer на готовых layout, вводе и тексте |
| **38** | Data Binding и On-Demand GUI | ⏳ Не начато | — | ObservableProperty<T>, ICommand, MVVM, инвалидирование layout/рисования и обновление только изменившегося интерфейса; без зависимости от полного LayerScript этапа 48 |
| **39** | КП-3: GUI-MVP на Windows | ⏳ Не начато | — | Одинаковое поведение GUI на DX12/Vulkan: ввод, фокус, текст, привязки, resize/DPI и совместный вывод куба и UI |
| **40** | КП-4: Linux (Vulkan) | ⏳ Не начато | — | Перенос и проверка куба + ZzzGUI-MVP; это первая обязательная Linux-приёмка, а не повтор этапа 31 |

### Уровень 4: Расширение рендера и скриптов

| № | Этап разработки | Статус | Файл этапа | Краткое описание |
|---|---|---|---|---|
| **41** | Расширение Camera и Frustum | ⏳ Не начато | — | Развитие базовой камеры из этапа 30, плоскости видимости и необходимые режимы камеры |
| **42** | Пространственные индексы и Frustum Culling | ⏳ Не начато | — | AABB, развитие ISpatialStorage вместо плоского DefaultSpatialStorage; выбор BVH/Octree по задаче и замерам, без обещания O(log N) для любого запроса видимости |
| **43** | Оптимизация кадровой очереди | ⏳ Не начато | — | Расширение работающих команд из этапа 30: результаты culling, сортировка и batching; не первая реализация Draw/RenderQueue |
| **44** | Развитие шейдеров и кэша PSO | ⏳ Не начато | — | Кэш и инвалидация поверх рабочего pipeline этапа 27/29: VkPipelineCache/ID3D12PipelineLibrary; офлайн-упаковка шейдеров относится к этапу 52 |
| **45** | Фасады движка для скриптов | ⏳ Не начато | — | EngineFacade/SceneFacade/ViewFacade, границы доступа поверх минимального API этапа 29; герметизация Engine.h и SDK-изоляция (TODO 24) |
| **46** | Изоляция и видимость скриптов | ⏳ Не начато | — | Политика доступа между уровнями скриптов; не обязательная зависимость базового CubeRotatorScript |
| **47** | Сериализуемые поля и ссылки (FieldRef) | ⏳ Не начато | — | Ссылки на GameObject, скрипты и ассеты, двухфазное разрешение; основа сохранения состояния при hot-reload |
| **48** | LayerScript и независимая кадровая частота слоёв | ⏳ Не начато | — | Layer3DScript/Layer2DScript/LayerMVVMScript, жизненный цикл, независимый FPS; развитие On-Demand из этапа 38. Здесь же определяется будущее поведение сохранённого `isActive` и эффективной активности в иерархии; 3D и эффекты в UI относятся к этапу 61 |

### Уровень 5: Ассеты, инструментарий и надёжность

| № | Этап разработки | Статус | Файл этапа | Краткое описание |
|---|---|---|---|---|
| **49** | Параметры импорта в .meta | ⏳ Не начато | — | Типоспецифичные параметры: mipmaps, сжатие, фильтрация, sRGB, настройки мешей и материалов; версия и миграция формата |
| **50** | Развитие пресетов сборки | ⏳ Не начато | — | Расширять реализованные в этапе 10 `build_settings/presets.json` и `platforms/<platform>/*.json` только по появившимся требованиям; не вводить заново старую `project_<platform>.json`-схему |
| **51** | Платформенные форматы ресурсов | ⏳ Не начато | — | Развитие общего MVP-формата и схемы выбора ресурсов; платформенные варианты без дублирования идентичности ассетов |
| **52** | Офлайн-компиляция ресурсов | ⏳ Не начато | — | Выбор и интеграция codec/checksum для payload `data.dat`/`.pak` с явной миграцией формата, расширенная оптимизация, зависимости и инвалидация результатов импорта (базовая компрессия текстур реализована в этапе 25) |
| **53** | Студия проектов | ⏳ Не начато | — | tools/editor и assets_builder: инспектор .zmat, FieldRef, безопасность unload/reload scripts.dll, состояние скриптов, режим сборки; долг TODO 2, 12–19 по актуальности |
| **54** | Доставка ресурсов в игровые таргеты | ⏳ Не начато | — | Развитие базовой Windows-доставки `package.dat`/`data.dat`/внешних `.pak`, необходимой уже в этапах 26–31: полнота отдельных файлов и поддержка остальных desktop/mobile таргетов |
| **55** | Развитие асинхронной загрузки сцен и отмена запросов | ⏳ Не начато | — | Базовая загрузка всех видов окон уже в этапе 8. Здесь дорабатываются владение задачами и безопасное завершение `SceneManager`, кооперативная отмена устаревших I/O-запросов ресурсов конвейера при смене/выгрузке сцены (TODO 26), состояния active/paused/unloading и диагностика |

### Уровень 6: Apple/mobile и полировка

| № | Этап разработки | Статус | Файл этапа | Краткое описание |
|---|---|---|---|---|
| **56** | КП-5: macOS и мобильные | ⏳ Не начато | — | Куб + ZzzGUI-MVP на macOS/iOS (Metal), Android (Vulkan); реализация недостающих GPU-ресурсов/pipeline, touch, DPI/Safe Area и Android resize (TODO 10) |
| **57** | SIMD для Mat4 | ⏳ Не начато | — | SSE2/AVX и NEON по замерам без изменения публичного API |
| **58** | CPU-ядра и пулы потоков (TaskDispatcher) | ⏳ В процессе | [stage_53_task_dispatcher_preview.md](stage_53_task_dispatcher_preview.md) | TaskDispatcher, P/E-ядра, affinity и приоритеты; CpuTopology |
| **59** | Прозрачность окна | ⏳ Не начато | — | Альфа окна и GAPI; Acrylic/Mica как нативная надстройка |
| **60** | Платформенный сплэшскрин | ⏳ Не начато | — | Win/Linux/macOS frameless splash, Android SplashScreen, iOS Storyboard |
| **61** | 3D и GPU-эффекты в UI | ⏳ Не начато | — | Viewport3DControl, интерактивные объекты, частицы, Glass/Acrylic blur |
| **62** | Адаптивный ZzzGUI | ⏳ Не начато | — | Desktop/mobile, стабильные Element ID, платформенные шаблоны и UI-скрипты |
| **63** | Высоконагруженный стриминг ресурсов (NVMe I/O и шардирование ResourceTable) | ⏳ Не начато | — | Развитие mapped-span и базового GPU-upload пути до платформенного асинхронного I/O/DirectStorage по измерениям; шардирование `ResourceTable<T>`; выделенная copy-очередь DX12, адаптивный backpressure, исследование Staging Ring-Buffer, chunked upload и стриминга открытого мира. Отложенная загрузка не меняет правило полной готовности обязательных ресурсов сцены без нового решения |
| **64** | Визуальные переходы сцен | ⏳ Не начато | — | Развитие каркаса из этапа 8: Fade/Cross-Fade/Dissolve/Wipe, маски, захват кадра |

### Уровень 7: Финал

| № | Этап разработки | Статус | Файл этапа | Краткое описание |
|---|---|---|---|---|
| **65** | Итоговая документация | ⏳ Не начато | — | Перенос актуальных решений в docs/, проверка ссылок и сохранности истории перед согласованным удалением current_plan/ |
