...
> **Текущая подзадача:** Расширение готового конвейера `ResourceManager`: единый асинхронный путь для Simple/MultiMesh, безопасная доставка callback'ов и реактивная цепочка готовности `GameObject -> GameLayer -> Scene -> SceneManager` без запуска незавершённой сцены.
> 
---
...
| **21** | GPU-буферы, загрузка в GPU и GPU-меш | ⏳ Не начато | — | Создание GPUBuffer (вершинный/индексный), upload-инфраструктура, барьеры, fenceValue/VkFence, неблокирующий staging lifetime, GPU-готовность Mesh; трехуровневый кэш (L1 GPU, L2 CPU, Ping-Pong очередь диска), единый источник правды классов сериализации (правило 19) |
| **22** | Базовые Shader и Material | ⏳ Не начато | — | Компиляция минимальных шейдеров DXIL/SPIR-V, рабочий pipeline для куба, MaterialData + Texture2D + параметры; разрешение всех обязательных зависимостей сцены и OnStart только после полной CPU/GPU-готовности |
