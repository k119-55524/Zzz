...
| **26** | Трёхсекционный TOC data.dat, внешние пакеты и сборщик PackagePacker | ⏳ Не начато | [`stage_26_multipak_toc_and_package_packer.md`](stage_26_multipak_toc_and_package_packer.md) | Трёхсекционный TOC `data.dat` (PackManifestTable, InlinePackageEntryTable, ExternalPackageEntryTable); автоматическое распределение контента по пакетам на основе типа ресурса без ручной нумерации; очистка целевого каталога перед сборкой; генерация внешних паков с префиксом типа `{:04d}_...pak`; атомарная публикация |
| **27** | Подготовка данных меша и последующий GPU upload | ⏳ Не начато | [`stage_27_gpu_buffers_and_upload.md`](stage_27_gpu_buffers_and_upload.md) | Чтение трёхсекционного `data.dat` и внешних `.pak`; массив владельцев `ReadOnlyFile` в `DataAssetsManager` и $O(1)$ доступ по `packIndex`; нормализация записей в `DataPackageEntry`; zero-allocation view/парсер над layout `MeshData`; запрос через `CpuResourceManager` с гарантированным lifetime; последующее создание GPUBuffer, staging-copy, submit/fence и настоящий GpuMesh |
