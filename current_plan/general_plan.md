...
| **19** | Плоский SoA-массив визуалов в NodeStorage и очистка GameObject | ⏳ Не начато | [`stage_19_visual_descriptors_soa_and_gameobject_cleanup.md`](stage_19_visual_descriptors_soa_and_gameobject_cleanup.md) | Замена `VisualPayload` на плоский `VisualDescriptor` (`meshGuid`, `materialGuid`) в `NodeStorage`; прямое соответствие `NodeHandle -> VisualDescriptor`; очистка `GameObject` от визуалов; единые флаги `Active/Visible` |
