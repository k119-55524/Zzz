...
| **24** | Постоянный archive handle (Memory-Mapped File) и Zero-Copy I/O конвейер архивов | ✅ Выполнено | [`stage_24_memory_mapped_archive_and_zero_copy_io.md`](stage_24_memory_mapped_archive_and_zero_copy_io.md) | Ликвидация системных вызовов в горячем пути; архитектура `core/io/storage/` с 3 классами: lock-free `ReadOnlyFile` (zero-copy mapping, системный `MappedFileHandle` выбирается CMake без `#ifdef`), потокобезопасный `ReadWriteFile` (`std::mutex`, `std::fstream`, C++23) и `FileSystemBase` (чистая топология путей и метаданных без байтового I/O); интеграция в `PackageArchive` и `DataAssetsManager`; `ArchivePayload` и передача `std::span` в `CpuResourceManager::GetAsync` |
