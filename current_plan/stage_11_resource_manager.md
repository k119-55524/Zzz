# Этап 11: Каркас ResourceManager и ResourceGarbageCollector (`ResourceManager`, `ResourceGarbageCollector`, `DoubleBufferedVector`)
...
- **Обоснование позиции в плане:** ResourceManager следует за форматами этапа 9 и предоставляет общий каркас для конкретных загрузчиков последующих этапов.
- **Статус:** Выполнен каркас по генплану. Сверка 2026-09-10: интерфейсы, очередь, I/O-поток, типизированные таблицы, GC и интеграция существуют. AddMesh/AddTexture/AddShader/AddMaterial ещё заглушки; маршрутизация хранилищ и CPU Mesh завершаются в этапе 16, GPU-ready/lifetime и остальные обязательные ресурсы — в этапах 19–20. Нижеприведённые снипеты являются историческим проектом API, не подтверждением полной реализации.
...
3. **Раздельный жизненный цикл при смене сцен (`UnloadSceneResources`):**
   - При выгрузке сцены снимается её владение Mesh/Texture2D/Material. Общие ресурсы сохраняются, пока нужны другим сценам или кадрам; upload lifetime вводится в будущем продолжении этапа 25, а lifetime ресурсов, использованных Draw, привязывается к frame fence в этапе 27.
   - Скомпилированные `Shader` **сохраняются в памяти**, так как они глобальны для игры и не требуют повторной компиляции.
4. **Асинхронная подготовка через CPU-контур без постоянного дубликата:**
   - `GpuResourceManager` получает исходные данные через `CpuResourceManager`. CPU-результат предоставляет стабильный pointer/span и удерживает backing memory; несжатые данные готовы к будущему staging-copy, сжатые в будущем проходят unpacker. Постоянная CPU-копия без отдельного потребителя не требуется.
...
6. **Интеграция с `Engine` и `SceneManager` при изоляции рендера:**
   ...
   - В пути отправки RenderFrame не выполняется поиск через ResourceManager и не читаются живые компоненты. PrepareFrame извлекает опубликованные матрицы и стабильные ссылки на ресурсы в данные кадрового слота (этап 27).
7. **Барьер параллельной загрузки (`Flush`):**
   - Текущий Flush ожидает завершения запросов CPU/I/O; это не обещание готовности GPU. Контракт GPU-ready/fence вводится в будущем GPU-upload продолжении этапа 25. Полная готовность всех обязательных графических зависимостей и `OnStart` доводятся после Material/Shader в этапе 26; ожидание не блокирует поток, необходимый для завершения запросов.
...
3. **Изоляция рендера (Least Privilege):**
   ...
   - Данные из сцены извлекаются в PrepareFrame; RenderFrame потребляет собственные команды и удержанные ресурсы кадрового слота. Конкретное представление определяется в этапе 27, обязательный MeshRenderer из старого наброска не навязывается существующей структуре GameObject.
...
Ниже сохранён исходный незаполненный чек-лист как история. Он не отменяет статус каркаса в генплане и не доказывает отсутствие существующих файлов. При ревизии 2026-09-10 новые сборки/прогоны не выполнялись; галочки проверки задним числом не выставляются. Актуальные незавершённые обязательства перечислены в статусе выше и в плане этапа 12; коммит только по отдельной команде.
