...
3. **Раздельный жизненный цикл при смене сцен (`UnloadSceneResources`):**
   - При выгрузке сцены снимается её владение Mesh/Texture2D/Material. Общие ресурсы сохраняются, пока нужны другим сценам или кадрам; фактический GPU-release разрешён только после завершения использующих их команд (этап 16).
...
4. **Прямая асинхронная загрузка в GPU (без дублирования на CPU):**
   - Постоянная CPU-копия GPU-ресурса без потребителя не требуется. Данные читаются из выбранного хранилища и передаются в GAPI; буфер декодирования живёт до последнего CPU-потребителя, upload/staging до GPU completion. Немедленно освобождать upload после записи команд нельзя.
...
6. **Интеграция с `Engine` и `SceneManager` при изоляции рендера:**
   ...
   - В пути отправки RenderFrame не выполняется поиск через ResourceManager и не читаются живые компоненты. PrepareFrame извлекает опубликованные матрицы и стабильные ссылки на ресурсы в данные кадрового слота (этап 20).
7. **Барьер параллельной загрузки (`Flush`):**
   - Текущий Flush ожидает завершения запросов CPU/I/O, а загрузчик обслуживается выделенным I/O-потоком; это не обещание параллельного upload на всех ядрах или готовности GPU. Контракт Ready/fence вводится в этапе 16. В этапе 18 публикация сцены и InvokeStart переводятся за готовность всех обязательных зависимостей; ожидание не блокирует поток, необходимый для завершения этих запросов.
...
3. **Изоляция рендера (Least Privilege):**
   ...
   - Данные из сцены извлекаются в PrepareFrame; RenderFrame потребляет собственные команды и удержанные ресурсы кадрового слота. Конкретное представление определяется в этапе 20, обязательный MeshRenderer из старого наброска не навязывается существующей структуре GameObject.
