...
6. **Интеграция с `Engine` и `SceneManager` при изоляции рендера:**
   ...
   - В пути отправки RenderFrame не выполняется поиск через ResourceManager и не читаются живые компоненты. PrepareFrame извлекает опубликованные матрицы и стабильные ссылки на ресурсы в данные кадрового слота (этап 24).
...
3. **Изоляция рендера (Least Privilege):**
   ...
   - Данные из сцены извлекаются в PrepareFrame; RenderFrame потребляет собственные команды и удержанные ресурсы кадрового слота. Конкретное представление определяется в этапе 24, обязательный MeshRenderer из старого наброска не навязывается существующей структуре GameObject.
