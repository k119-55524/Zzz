...
- **Следующий потребитель:** этап 25 использует готовый resource/I/O фундамент независимо от сценовой топологии; этап 27 после update формирует неизменяемые render-команды кадра `N`, исполняемые render-потоком как `N-1`.
...
BeginFrame слоя
  ...
  -> в этапе 27 публикация immutable render-команд N
```
...
4. Render thread не получает доступ к mutable `NodeStorage`. Снимок команд `N/N-1` реализуется в этапе 27 после завершения update; в этапе 18 не вводятся половинчатые thread-safe обёртки.
...
- Копирование/двойная буферизация render-команд и исполнение `N-1` — этап 27.
