...
- **Следующий потребитель:** этап 19 использует готовые CPU-ресурсы независимо от сценовой топологии; этап 23 подключает скриптовый lifecycle; этап 24 после update формирует неизменяемые render-команды кадра `N`, исполняемые render-потоком как `N-1`.
...
BeginFrame слоя
  ...
  -> в этапе 24 публикация immutable render-команд N
```
...
4. Render thread не получает доступ к mutable `NodeStorage`. Снимок команд `N/N-1` реализуется в этапе 24 после завершения update; в этапе 18 не вводятся половинчатые thread-safe обёртки.
...
4. Отдельные `GameObject` пока остаются heap-объектами, чтобы не менять адреса, на которые ссылаются скриптовые заготовки. Устранение этих аллокаций и полноценный скриптовый hot path относятся к этапу 23.
...
- [x] Начальная сборка topology и всех world matrices выполняется линейно и без рекурсии.
- [x] Для каждого узла соблюдаются `parentIndex < childIndex` и корректный непрерывный `subtreeEnd`.
- [x] Dirty tree выделяет память пропорционально фактической ёмкости и не имеет скачка полного уровня на 4096/4097.
- [x] Dirty-биты переживают `BeginFrame` и очищаются только при consume.
- [x] Перекрывающиеся dirty-поддеревья не приводят к повторному пересчёту узла.
- [x] Повторный transform update не делает heap allocations после подготовки ёмкости.
- [x] `NodeStorage` является единственным источником TRS/world; домены и spatial не содержат устаревающих копий.
- [x] Каждый node связан ровно с одним Object/Entity domain через `domainKind/domainHandle`.
- [x] `layerObjectIndex` удалён из runtime bindings.
- [x] В spatial зарегистрированы только `HasMesh()` nodes, независимо от `isEntity` и сохранённого `isActive`.
- [x] Spatial handle не отождествляется с node index; узлы без меша имеют invalid spatial handle.
- [x] Плоский spatial читается allocation-free как span handles.
- [x] `NodeStorage` живёт дольше ссылающихся на него `GameObject`.
- [x] Слой успешно создаётся из упакованной сцены этапа 17, а существующий executable доходит до начала/завершения `Populate` без новых ошибок и предупреждений.
- [x] Результаты фактически выполненной сборки/прогона записаны с датой и конфигурацией.
- [x] Пользователь явно подтвердил: «Шаг принят».
...
- Жизненный цикл скриптов, GUID-резолв объектов для скриптов, пользовательский `OnStart/OnUpdate/OnDestroy` и hot reload — этап 23.
- Полное устранение отдельных heap allocations `GameObject` — этап 23 после фиксации скриптового ownership.
- Копирование/двойная буферизация render-команд и исполнение `N-1` — этап 24.
