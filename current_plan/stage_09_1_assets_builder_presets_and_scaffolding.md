...
- Генерирует валидный скелет: `project.json` (с `company_name`, `app_name`), `Assets/`, `Scripts/`, `build_settings/presets.json` и `build_settings/platforms/{windows,android,...}/default_config.json`.
...
2. **Ядро сборщика (`assets_builder_lib`):** [✅ Выполнено]
   ...
   - `ProjectCreator` (создание структуры нового проекта в пустой папке).
...
5. **Тестирование и верификация (TODO):** [⏳ В процессе]
   - Проверка создания каркаса в новой пустой папке (`ProjectCreator`).
