# Этап 12: Интерфейсы доменов и пространственное хранилище (Domain Interfaces & Spatial Storage)

## 1. Контекст и цели этапа
- **Номер пункта:** **Пункт 12** (Уровень 2: GAPI-ресурсы, содержимое куба и сквозной рендер).
...
- **Статус:** Выполнено по генплану. Интеграция доменов/слоёв продолжена в этапе 15, их runtime-связи уточняются в этапе 18; продвинутые пространственные индексы отложены в этап 34 и не блокируют куб или GUI-MVP.
...
В соответствии с **Правилом 17** (строгая очерёдность этапов: `Camera`/`Frustum` — этап 33, `RenderQueue` — этапы 22/35), на **Этапе 12** интерфейс `ISpatialStorage` реализует фундамент пространственного хранения над существующими типами (`::zzz::math::Vec3<zF32>`, `uint64_t userData`), без преждевременного введения несуществующих классов:
...
namespace zzz::engine
{
    ...
    public:
        ...
        // - QueryFrustum(const Camera&) появится на Этапе 34 (пространственные индексы и Frustum Culling)
        // - Raycast не входит в текущий генеральный план и добавляется только при появлении этапа физики
    };
}
```
