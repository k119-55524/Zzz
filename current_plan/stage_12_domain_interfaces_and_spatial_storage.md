...
- **Статус:** Выполнено по генплану. Интеграция доменов/слоёв продолжена в этапе 15; продвинутые пространственные индексы отложены в этап 32 и не блокируют куб или GUI-MVP.
...
В соответствии с **Правилом 17** (строгая очерёдность этапов: `Camera`/`Frustum` — этап 31, `RenderQueue` — этапы 20/33), на **Этапе 12** интерфейс `ISpatialStorage` реализует фундамент пространственного хранения над существующими типами (`::zzz::math::Vec3<zF32>`, `uint64_t userData`), без преждевременного введения несуществующих классов:
