...
- **Статус:** Выполнено по генплану. Интеграция доменов/слоёв продолжена в этапе 15, их runtime-связи уточняются в этапе 18; продвинутые пространственные индексы отложены в этап 35 и не блокируют куб или GUI-MVP.
...
В соответствии с **Правилом 17** (строгая очерёдность этапов: `Camera`/`Frustum` — этап 34, `RenderQueue` — этапы 23/36), на **Этапе 12** интерфейс `ISpatialStorage` реализует фундамент пространственного хранения над существующими типами (`::zzz::math::Vec3<zF32>`, `uint64_t userData`), без преждевременного введения несуществующих классов:
...
namespace zzz::engine
{
    ...
    public:
        ...
        // - QueryFrustum(const Camera&) появится на Этапе 35 (пространственные индексы и Frustum Culling)
