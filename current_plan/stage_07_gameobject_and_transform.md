...
- **Статус:** Выполнено по генплану; исторический этап. Transform, владение иерархией и ObjectWorld заменены этапами 12–15. Нижеприведённые старые структуры не являются заданием восстановить удалённый код. Актуальные контракты кадра: генплан, правило 14.
...
2. **Иерархия сцены в `GameObject` (Parent / Children):**
   ...
   - При смене родителя с сохранением мирового положения для принятого row-vector порядка: $M_{local\_new} = M_{world\_old} \times M_{parent\_world\_new}^{-1}$.
3. **Ленивый пересчёт матриц (Dirty Flag Pattern):**
   - Матрица мира $M_{world} = M_{local} \times M_{parent\_world}$. Вычисление по требованию было моделью этапа 7; после этапа 14 расчёт выполняется в NodeStorageBlock::ResolveTransforms.
...
11. **Согласованность с Правилом 13 («Строгая последовательность зависимостей») и Правилом 30 («YAGNI»):**
    - На шаге 7 не вводились зависимости от ещё не реализованных Camera и RenderQueue. По актуальному генплану их минимальный путь входит в этап 20; Frustum/Culling в этапах 31–32.
...
12. **Многопоточная изоляция кадра через неизменяемый снимок (Immutable Render Snapshot, Правило 28):**
    ...
    - Минимальная экстракция опубликованных сценовых данных в кадровые команды реализуется в этапе 20, оптимизация очереди в этапе 33.
    - Актуальный путь: Secondary -> CPU Handover Barrier -> Primary -> PrepareFrame -> собственные команды кадрового слота. RenderFrame не читает изменяемые объекты сцены; способ передачи не требует конкретного SwapQueue API из старого наброска.
    - Безопасность определяется владением, завершением CPU-читателей и GPU-fences, а не самим наличием двух буферов (генплан, правило 14).
