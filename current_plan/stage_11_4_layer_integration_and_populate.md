...
3. **Локальный барьер сдачи кадра (Handover Barrier) перед `PostRender()`:**
   - **Применение отложенных мутаций:** пакетное применение `m_ReparentQueue`, `m_DeleteQueue` и Staging-спавна к топологии в один последовательный проход без риска реентрантности.
   - **Каскадный расчёт:** обновление мировых матриц по битовой маске `dirtyTracker` и синхронизация ячеек `ISpatialStorage`.
   - **Очистка трекера:** векторный сброс `dirtyTracker` при наличии изменений (<0.05 мс).
