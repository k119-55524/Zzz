...
- **Статус:** ✅ Выполнено. Документ сохраняет исторический скоуп этапа; последующие изменения из этапов 23–25 имеют приоритет над отдельными формулировками ниже.
...
> [!IMPORTANT]
> Разделы ниже описывают состояние и решения именно этапа 22, а не текущую архитектуру. Этап 23 удалил `IoScheduler`/`m_IoThread`, этап 24 заменил копирующее чтение на постоянные mapped spans, а текущая подзадача этапа 25 сохраняет маршрут `GpuResourceManager → CpuResourceManager → DataAssetsManager` и доводит его до стабильных pointers/spans, готовых для будущего staging-copy. Источником истины является [`stage_25_gpu_buffers_and_upload.md`](stage_25_gpu_buffers_and_upload.md).
...
- **Без `Invalidate/Retry` и без поколений (`generation`):** Записи в `ResourceTable` неизменяемы и не удаляются до shutdown приложения (`Clear()`). Поддержка отмены и инвалидации запросов отложена до этапа 52; hot-reload ресурсов требует отдельного решения.
- **Постоянные read-only представления архивов:** не входили в этап 22 и впоследствии реализованы этапом 24 через `ReadOnlyFile` / `MappedFileHandle`.
- **Без `Priority Promotion` и `Fair Scheduling`:** Используется стандартная двухприоритетная очередь `High` $\to$ `Background`.
- **Без аппаратного `GpuUploadScheduler`:** текущая подзадача этапа 25 также не реализует GPU upload; сначала подготавливается CPU-результат, пригодный для будущего staging-copy.
- **Без шардирования `ResourceTable`:** Единый `std::shared_mutex` с Read-First поиском достаточен для базовых сцен; шардирование (Striped Locks) зафиксировано в TODO 27 (этап 60).
- **Без `ExecuteBudgeted`:** Главный поток на старте сбрасывает все колбэки через `ExecuteAll()`. Бюджетирование кадра относится к последующему развитию загрузки сцен и высоконагруженного стриминга (этапы 52/60).
...
- Создать `GpuResourceBuilder.h` со специализацией `Build` для меша, текстуры, материала, шейдера (создание RAM-заглушек без вызовов GAPI; текущая подзадача этапа 25 сохраняет получение данных через `CpuResourceManager`, а реальный upload остаётся будущим продолжением).
