# Этап 10: Менеджер проектов Сборщика Ассетов: каркас проекта, валидация структуры, пресеты сборки и платформенные конфиги

**Статус:** ⏳ В процессе  

> Ревизия 2026-09-10: основная реализация существует, приёмочные сценарии раздела 3.5 остаются открытыми (TODO 6) и должны быть закрыты до КП-1. Эта правка не подтверждает новые прогоны. Этап 44 развивает нынешние build_settings/presets.json, а не создаёт пресеты повторно.

**Цель этапа:** Менеджер существующих проектов: строгая валидация без перезаписи повреждённых данных, платформенные настройки в `build_settings/platforms/`, пресеты `build_settings/presets.json`, выбор конфигураций в GUI/CLI. Scaffolding был исключён из реализации (раздел 2.2); не входит в текущую приёмку.
...
4.1. **Ревизия архитектуры (2026-09-06):** [✅ Выполнено] Функциональность не менялась, устранены находки аудита:
   - Удалён мёртвый код: `ProjectCreator.cs` (см. п.2.2) и весь каталог `assets_builder_lib/Serializers/`
     ...
     в корне репозитория; это историческое размещение удалённого из рабочего кода материала.
   ...
   - Уточнение по коду 2026-09-10 (TODO 16-19): AssetsBuilderEngine.BuildPackage и GetVersion уже удалены.
     Остались сверка PInvoke и реальных экспортов, ревизия общей оркестрации GUI/headless и разделение
     PackagePacker по ответственности при развитии инструментария. Обёртки GetDataPackage без потребителя
     не нужны; парсинг JSON разных платформ не является утечкой нативного SDK. Нужные текущей сборке
     импорты проверяются в этапе 10, отложенная очистка относится к этапу 47.
