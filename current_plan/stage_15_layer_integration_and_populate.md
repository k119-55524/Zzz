...
4. **Очистка мёртвого кода в `SceneTreeContainer`:**
   ...
   - Это заменяет требование очередей из этапа 14, но не решает безопасное удаление из исполняемого callback. Этот обязательный контракт доводится вместе с lifecycle в этапе 23.
