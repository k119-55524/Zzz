...
2. **Параллельная фаза во `ViewManager::Update()`:**
   ...
   - PrepareFrame предназначен для чтения опубликованного Front Buffer и записи собственных команд слота; фактические BuildRenderTree/SubmitRenderTree ещё заглушки, доводятся в этапе 22.
...
4. **Очистка мёртвого кода в `SceneTreeContainer`:**
   ...
   - Это заменяет требование очередей из этапа 14, но не решает безопасное удаление из исполняемого callback. Этот обязательный контракт доводится вместе с lifecycle в этапе 21.
