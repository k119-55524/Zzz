...
## Сохранение порядка вызова событий при `SetActive(false/true)`
В `BaseScript::SetActive(bool active)` при выключении скрипт вызывает `OnUnbindEvents()` (отписка), а при повторном включении `OnBindEvents()` (подписка). Из-за того, что `Subscribe()` всегда добавляет колбэк в конец вектора, порядок вызова подписчиков ломается (`ScriptA` уходит в самый конец относительно `ScriptB` и `ScriptC`).

Необходимо:
- Разработать механизм сохранения оригинальной позиции подписки (например, флаг паузы/деактивации `isPaused` внутри `CallbackEntry` без физического удаления из вектора при временно выключенном скрипте).
