...
namespace zzz::engine
{
	...
	public:
		...
		GpuMesh_DX(const Guid& guid, std::string name, ResourceRef<CpuMesh> cpuMesh)
			: ResourceBase(guid, eResourceType::Mesh, std::move(name)), m_CpuMesh(std::move(cpuMesh)) {}
		~GpuMesh_DX() override = default;

		[[nodiscard]] static std::shared_ptr<GpuMesh_DX> CreateFromCpu(ResourceRef<CpuMesh> cpuMesh)
		{
			return safe_make_shared<GpuMesh_DX>(cpuMesh->GetGuid(), std::string(cpuMesh->GetName()), std::move(cpuMesh));
		}
