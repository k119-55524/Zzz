...
#pragma region Window Events
		...
		// Вызывается, когда пользователь начинает перетаскивать рамку окна (захватил мышью).
		std::function<void()> OnResizeStart;

		// Вызывается непрерывно в процессе перетаскивания рамки окна пользователем.
		std::function<void()> OnSizing;

		// Вызывается, когда пользователь отпускает рамку окна после изменения размера.
