...
{
	/**
	 * @brief Платформенная реализация IMonitorProvider для macOS.
	 */
	class MonitorProviderMacOS final : public IMonitorProvider
	{
	public:
		MonitorProviderMacOS()
		{
			RefreshMonitors();
		}

		~MonitorProviderMacOS() override = default;
		...
		{
			m_Monitors.clear();
			m_Monitors.emplace_back("MACOS_DISPLAY_0", "macOS Main Display", Size2D<zU32>{1920, 1080}, 0, 0, true, 2.0f);
		}
