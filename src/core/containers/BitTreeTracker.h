...
namespace zzz::core
{
	...
	public:
		...
		/// @param outIndices Буфер для записи индексов (очищается перед сбором).
	...
	private:
		...
		void TraverseLevel(uint32_t level, size_t wordIndexInLevel, std::vector<uint32_t>& outIndices) const;

		uint32_t              m_Capacity{ 0 };
		uint32_t              m_Depth{ 1 };
		std::vector<uint64_t> m_Words; // ЕДИНСТВЕННЫЙ непрерывный массив пирамиды
	};
}
