...
namespace zzz::core
{
	...
	public:
		...
		/**
		 * @brief Выполняет мягкую проверку сохраненных выборов GPU и Монитора против актуального среза железа.
		 * Если сохраненное устройство отсутствует, его ID обнуляется (сигнализирует о необходимости нового выбора).
		 */
		void CheckAndApplySoftChanges(const PlatformHardwareState& actualState)
		{
			// 1. Проверка GPU
			bool selectedGpuExists = false;
			if (!m_SelectedGpuId.empty())
			{
				for (const auto& gpu : actualState.m_Gpus)
				{
					if (gpu.GetPlatformGpuId() == m_SelectedGpuId)
					{
						selectedGpuExists = true;
						break;
					}
				}
			}

			if (!selectedGpuExists)
			{
				m_SelectedGpuId.clear(); // Очищаем ID — сигнализирует о необходимости автовыбора GPU
				DOut("[PlatformHardwareState] Изменение конфигурации видеокарт (GPU). Необходим выбор графического адаптера.");
			}

			// 2. Проверка Мониторов
			if (actualState.m_Monitors.size() > 1)
			{
				bool selectedMonitorExists = false;
				if (!m_SelectedMonitorId.empty())
				{
					for (const auto& mon : actualState.m_Monitors)
					{
						if (mon.GetPlatformMonitorId() == m_SelectedMonitorId)
						{
							selectedMonitorExists = true;
							break;
						}
					}
				}

				if (!selectedMonitorExists)
				{
					m_SelectedMonitorId.clear(); // Очищаем ID — сигнализирует о необходимости выбора монитора
					DOut("[PlatformHardwareState] Изменение конфигурации мониторов. Необходим выбор целевого экрана.");
				}
			}
			else if (actualState.m_Monitors.size() == 1)
			{
				// Если монитор всего один — выставляем его ID
				m_SelectedMonitorId = actualState.m_Monitors[0].GetPlatformMonitorId();
			}

			// 3. Обновляем списки оборудования на актуальные
			m_Cpus = actualState.m_Cpus;
			m_Ram = actualState.m_Ram;
			m_Motherboard = actualState.m_Motherboard;
			m_Gpus = actualState.m_Gpus;
			m_Monitors = actualState.m_Monitors;
			m_Storages = actualState.m_Storages;
			m_NetworkAdapters = actualState.m_NetworkAdapters;
		}
