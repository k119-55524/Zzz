...
export namespace zzz::dx
{
	...
	public:
		...
		Result<> InitializeByText(std::string&& srcVS, std::string&& srcPS);
		...
		ComPtr<ID3DBlob> GetVS() const noexcept { return m_VS; }
		ComPtr<ID3DBlob> GetPS() const noexcept { return m_PS; }
	...
	Result<> Shader_DirectX::InitializeByText(std::string&& srcVS, std::string&& srcPS)
	{
		...
		auto resVS = CompileShaderFromSource(srcVS.c_str(), L"mainVS", m_GAPI->GetGapiSupportChecker().GetHighestShaderModelAsString(eShaderType::Vertex));
		if (!resVS)
			return resVS.error();

		auto resPS = CompileShaderFromSource(srcPS.c_str(), L"mainPS", m_GAPI->GetGapiSupportChecker().GetHighestShaderModelAsString(eShaderType::Pixel));
		if (!resPS)
			return resPS.error();
