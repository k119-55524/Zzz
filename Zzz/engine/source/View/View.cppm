...
namespace zzz
{
	...
	public:
		...
		[[nodiscard]] inline void SetFullScreenState(bool fss) { return m_Window->SetFullScreenState(fss); };
		[[nodiscard]] inline bool GetFullScreenState() const { return m_Window->GetFullScreenState(); };
	...
	void View::OnViewResize(const Size2D<>& size, eTypeWinResize resizeType)
	{
		...
		case eTypeWinResize::Show:
			DebugOutputLite(L">>>>> Show app window - {}x{}.", std::to_wstring(size.width), std::to_wstring(size.height));
			break;
		case eTypeWinResize::Resize:
			DebugOutputLite(L">>>>> Resize app window to {}x{}.", std::to_wstring(size.width), std::to_wstring(size.height));
