...
namespace zzz
{
	...
	{
	public:
		Result<std::shared_ptr<AppConfig>> GetAppWinConfig(const std::shared_ptr<ZamlConfig> zaml)
		{
			...
			if (caption && className && width && height && icoPath && icoSize)
			{
				...
				config = safe_make_shared<AppConfig>(
					caption.value(),
					className.value(),
					winSize,
					icoPath.value(),
			...
			return Result<std::shared_ptr<AppConfig>>(config);
		}
	};
}
#endif