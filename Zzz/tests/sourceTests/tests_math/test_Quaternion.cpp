...
Result<std::string> test_Quaternion::Run()
{
	...
	// Test 1: Конструкторы
	{
		...
		if (!quatEquals(q1, Quaternion(0, 0, 0, 1)))
			return UNEXPECTED(eResult::failure, L"Failed: default constructor (identity)");
	...
	// Test 2: Identity
	{
		...
		if (!quatEquals(id, Quaternion(0, 0, 0, 1)))
			return UNEXPECTED(eResult::failure, L"Failed: identity()");
	}
	...
	// Test 4: Normalization
	{
		...
		if (!quatEquals(zn, Quaternion::identity()))
			return UNEXPECTED(eResult::failure, L"Failed: normalize zero quaternion");
	}
	...
	// Test 6: Conjugate
	{
		...
		if (!quatEquals(c, Quaternion(-1, -2, -3, 4)))
			return UNEXPECTED(eResult::failure, L"Failed: conjugate");
	}
	...
	// Test 7: Inverse
	{
		...
		if (!quatEquals(prod, Quaternion::identity()))
			return UNEXPECTED(eResult::failure, L"Failed: inverse");
	}
	...
	// Test 8: Quaternion multiplication (identity)
	{
		...
		if (!quatEquals(r, q))
			return UNEXPECTED(eResult::failure, L"Failed: q * identity");

		Quaternion l = Quaternion::identity() * q;
		if (!quatEquals(l, q))
			return UNEXPECTED(eResult::failure, L"Failed: identity * q");
	}
	...
	// Test 9: Axis-angle
	{
		...
		if (!quatEquals(q, Quaternion::identity()))
			return UNEXPECTED(eResult::failure, L"Failed: axis-angle zero rotation");
	}
	...
	// Test 10: Vector rotation (используем operator*)
	{
		...
		if (!floatEquals(r.x(), 0.0f) ||
			...
			return UNEXPECTED(eResult::failure, L"Failed: vector rotation");
	}
	...
	// Test 11: Euler angles (используем fromEulerXYZ)
	{
		...
		if (!floatEquals(fwd.x(), 1.0f) ||
			...
			return UNEXPECTED(eResult::failure, L"Failed: fromEulerXYZ");
	}
	...
	// Test 12: Equality
	{
		...
		if (a != b)
			return UNEXPECTED(eResult::failure, L"Failed: inequality (equal)");
		if (a == c)
			return UNEXPECTED(eResult::failure, L"Failed: inequality (different)");
	}
	...
	// Test 13: Arithmetic operators
	{
		...
		if (!quatEquals(neg, Quaternion(-1, -2, -3, -4)))
			return UNEXPECTED(eResult::failure, L"Failed: operator- (unary)");

		// Scalar multiplication
		Quaternion scaled = a * 2.0f;
		if (!quatEquals(scaled, Quaternion(2, 4, 6, 8)))
			return UNEXPECTED(eResult::failure, L"Failed: operator* (scalar)");
	...
	// Test 15: toEulerAngles roundtrip via vectors (ZYX order: yaw, pitch, roll)
	{
		...
		if (!checkEulerZYX(0.0f, 0.5f, 0.0f)) // pure pitch
			return UNEXPECTED(eResult::failure, L"Failed: toEulerAngles pitch");

		if (!checkEulerZYX(0.5f, 0.0f, 0.0f)) // pure yaw
			return UNEXPECTED(eResult::failure, L"Failed: toEulerAngles yaw");

		if (!checkEulerZYX(0.0f, 0.0f, 0.5f)) // pure roll
			return UNEXPECTED(eResult::failure, L"Failed: toEulerAngles roll");

		if (!checkEulerZYX(0.3f, 0.5f, 0.7f)) // combined angles
			return UNEXPECTED(eResult::failure, L"Failed: toEulerAngles combined vectors");

		if (!checkEulerZYX(0.0f, 0.0f, 0.0f)) // identity
			return UNEXPECTED(eResult::failure, L"Failed: toEulerAngles identity");
	}
	...
	// Test 17: getForward / getUp / getRight
	{
		...
		if (!floatEquals(fwd.x(), 0.0f) || !floatEquals(fwd.y(), 0.0f) || !floatEquals(fwd.z(), 1.0f))
			return UNEXPECTED(eResult::failure, L"Failed: getForward (identity)");

		Vector3 up = q.getUp();
		if (!floatEquals(up.x(), 0.0f) || !floatEquals(up.y(), 1.0f) || !floatEquals(up.z(), 0.0f))
			return UNEXPECTED(eResult::failure, L"Failed: getUp (identity)");

		Vector3 right = q.getRight();
		if (!floatEquals(right.x(), 1.0f) || !floatEquals(right.y(), 0.0f) || !floatEquals(right.z(), 0.0f))
			return UNEXPECTED(eResult::failure, L"Failed: getRight (identity)");
	}
	...
	// Test 19: fromToRotation
	{
		...
		if (!floatEquals(result.x(), to.x()) ||
			...
			return UNEXPECTED(eResult::failure, L"Failed: fromToRotation");
	}
	...
	// Test 20: slerp
	{
		...
		if (!mid.isNormalized())
			return UNEXPECTED(eResult::failure, L"Failed: slerp normalization");

		Quaternion start = Quaternion::slerp(a, b, 0.0f);
		if (!quatEquals(start, a))
			return UNEXPECTED(eResult::failure, L"Failed: slerp t=0");

		Quaternion end = Quaternion::slerp(a, b, 1.0f);
		if (!quatEquals(end, b))
			return UNEXPECTED(eResult::failure, L"Failed: slerp t=1");
	}
	...
	// Test 21: nlerp
	{
		...
		if (!mid.isNormalized())
			return UNEXPECTED(eResult::failure, L"Failed: nlerp normalization");
	}
	...
	// Test 22: isValid
	{
		...
		if (!valid.isValid())
			return UNEXPECTED(eResult::failure, L"Failed: isValid (valid quaternion)");

		Quaternion invalid(std::numeric_limits<float>::quiet_NaN(), 0, 0, 1);
		if (invalid.isValid())
			return UNEXPECTED(eResult::failure, L"Failed: isValid (NaN quaternion)");
	}
	...
	// Test 23: fromLookRotation
	{
		...
		if (!floatEquals(resultFwd.x(), forward.x()) ||
			...
			return UNEXPECTED(eResult::failure, L"Failed: fromLookRotation");
	}
