...
Result<std::string> test_Vector4::Run()
{
	...
	// Test 2: Getters и setters
	{
		...
		if (!floatEquals(v.r(), 1) || !floatEquals(v.g(), 2) ||
			...
			return UNEXPECTED(eResult::failure, L"Failed: RGBA getters");
		...
		if (!vectorEquals(v, Vector4(10, 20, 30, 40)))
			return UNEXPECTED(eResult::failure, L"Failed: Component setters");
		...
		if (!vectorEquals(v, Vector4(5, 6, 7, 8)))
			return UNEXPECTED(eResult::failure, L"Failed: RGBA setters");
	}
	...
	// Test 4: Арифметические операции
	{
		...
		if (!vectorEquals(compMul, Vector4(5, 12, 21, 32)))
			return UNEXPECTED(eResult::failure, L"Failed: Component-wise multiplication");

		// Покомпонентное деление
		Vector4 compDiv = v2 / v1;
		if (!vectorEquals(compDiv, Vector4(5, 3, 7.0f / 3.0f, 2)))
			return UNEXPECTED(eResult::failure, L"Failed: Component-wise division");

		// Унарный минус
		Vector4 negated = -v1;
		if (!vectorEquals(negated, Vector4(-1, -2, -3, -4)))
			return UNEXPECTED(eResult::failure, L"Failed: Unary negation");
	}
	...
	// Test 5: Операции присваивания
	{
		...
		if (!vectorEquals(v, Vector4(2, 3, 4, 5)))
			return UNEXPECTED(eResult::failure, L"Failed: += operator");
		...
		if (!vectorEquals(v, Vector4(2, 4, 6, 8)))
			return UNEXPECTED(eResult::failure, L"Failed: *= scalar operator");
		...
		v *= Vector4(2, 2, 2, 2);
		if (!vectorEquals(v, Vector4(2, 4, 6, 8)))
			return UNEXPECTED(eResult::failure, L"Failed: *= vector operator");
	...
	// Test 6: Операции сравнения
	{
		...
		if (!(v1 == v2))
			return UNEXPECTED(eResult::failure, L"Failed: Equality operator (equal vectors)");

		if (v1 == v3)
			return UNEXPECTED(eResult::failure, L"Failed: Equality operator (different vectors)");

		if (v1 != v2)
			return UNEXPECTED(eResult::failure, L"Failed: Inequality operator (equal vectors)");

		if (!(v1 != v3))
			return UNEXPECTED(eResult::failure, L"Failed: Inequality operator (different vectors)");
	}
	...
	// Test 7: Длина и нормализация
	{
		...
		if (!floatEquals(normalized.length(), 1.0f))
			return UNEXPECTED(eResult::failure, L"Failed: normalized (length not 1)");

		if (!vectorEquals(normalized, Vector4(0.6f, 0.8f, 0, 0)))
			return UNEXPECTED(eResult::failure, L"Failed: normalized (wrong direction)");

		Vector4 v2(3, 4, 0, 0);
		v2.normalize();
		if (!floatEquals(v2.length(), 1.0f))
			return UNEXPECTED(eResult::failure, L"Failed: normalize (length not 1)");

		// Нормализация нулевого вектора
		Vector4 zero;
		Vector4 normalizedZero = zero.normalized();
		if (!vectorEquals(normalizedZero, Vector4(0, 0, 0, 0)))
			return UNEXPECTED(eResult::failure, L"Failed: normalized zero vector");
	}
	...
	// Test 8: Скалярное произведение
	{
		...
		if (!floatEquals(dotProduct, 70.0f))
			return UNEXPECTED(eResult::failure, L"Failed: dot product");

		// Перпендикулярные векторы
		Vector4 v3(1, 0, 0, 0);
		Vector4 v4(0, 1, 0, 0);
		if (!floatEquals(v3.dot(v4), 0.0f))
			return UNEXPECTED(eResult::failure, L"Failed: dot product (perpendicular)");
	}
	...
	// Test 9: Линейная интерполяция
	{
		...
		if (!vectorEquals(lerp0, v1))
			return UNEXPECTED(eResult::failure, L"Failed: lerp at t=0");

		Vector4 lerp1 = Vector4::lerp(v1, v2, 1.0f);
		if (!vectorEquals(lerp1, v2))
			return UNEXPECTED(eResult::failure, L"Failed: lerp at t=1");

		Vector4 lerp05 = Vector4::lerp(v1, v2, 0.5f);
		if (!vectorEquals(lerp05, Vector4(5, 5, 5, 5)))
			return UNEXPECTED(eResult::failure, L"Failed: lerp at t=0.5");
	}
	...
	// Test 10: Clamp
	{
		...
		if (!vectorEquals(clamped, Vector4(10, 15, 20, 20)))
			return UNEXPECTED(eResult::failure, L"Failed: clamp (vector bounds)");

		Vector4 clamped2 = v.clamp(10.0f, 20.0f);
		if (!vectorEquals(clamped2, Vector4(10, 15, 20, 20)))
			return UNEXPECTED(eResult::failure, L"Failed: clamp (scalar bounds)");
	}
	...
	// Test 11: Component-wise min/max
	{
		...
		if (!vectorEquals(minVec, Vector4(1, 2, 3, 1)))
			return UNEXPECTED(eResult::failure, L"Failed: compMin");

		Vector4 maxVec = v1.compMax(v2);
		if (!vectorEquals(maxVec, Vector4(4, 5, 6, 7)))
			return UNEXPECTED(eResult::failure, L"Failed: compMax");
	}
	...
	// Test 12: Abs
	{
		...
		if (!vectorEquals(absVec, Vector4(1, 2, 3, 4)))
			return UNEXPECTED(eResult::failure, L"Failed: abs");
	}
	...
	// Test 13: Min/Max component
	{
		...
		if (!floatEquals(minComp, 2.0f))
			return UNEXPECTED(eResult::failure, L"Failed: minComponent");

		float maxComp = v.maxComponent();
		if (!floatEquals(maxComp, 8.0f))
			return UNEXPECTED(eResult::failure, L"Failed: maxComponent");
	}
	...
	// Test 14: Reciprocal
	{
		...
		if (!vectorEquals(recip, Vector4(0.5f, 0.25f, 0.2f, 0.1f)))
			return UNEXPECTED(eResult::failure, L"Failed: reciprocal");
	}
	...
	// Test 15: Sqrt
	{
		...
		if (!vectorEquals(sqrtVec, Vector4(2, 3, 4, 5)))
			return UNEXPECTED(eResult::failure, L"Failed: sqrt" );
	}
	...
	// Test 16: Rsqrt (обратный квадратный корень)
	{
		...
		if (!rsqrtEquals(rsqrtVec[0], 0.5f) ||
			...
			return UNEXPECTED(eResult::failure, L"Failed: rsqrt");
	}
	...
	// Test 18: Cross product 3D
	{
		...
		if (!vectorEquals(cross, Vector4(0, 0, 1, 0)))
			return UNEXPECTED(eResult::failure, L"Failed: cross3 (basic)");

		// Проверка антикоммутативности: a х b = -(b х a)
		Vector4 crossRev = v2.cross3(v1);
		if (!vectorEquals(crossRev, Vector4(0, 0, -1, 0)))
			return UNEXPECTED(eResult::failure, L"Failed: cross3 (anticommutativity)");
		...
		if (!vectorEquals(cross2, Vector4(-3, 6, -3, 0)))
			return UNEXPECTED(eResult::failure, L"Failed: cross3 (complex)");
	}
	...
	// Test 19: Scalar multiplication from left
	{
		...
		if (!vectorEquals(scaled, Vector4(2, 4, 6, 8)))
			return UNEXPECTED(eResult::failure, L"Failed: left scalar multiplication");
	}
