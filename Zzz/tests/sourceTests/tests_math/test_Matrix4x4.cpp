	...
	const wchar_t* testName)
{
	...
	if (std::abs(result[0] - expectedResult[0]) > epsilon)
	{
		...
		return UNEXPECTED(eResult::failure, msg);
	}
...
Result<std::string> test_Matrix4x4::ValidatePerspective_DirectX()
{
	...
	for (const auto& testCase : testCases)
	{
		...
		if (!result)
		{
			...
			return UNEXPECTED(eResult::failure, errorMsg);
		}
	}
	...
	float farZ)
{
	...
	if (std::abs(proj[0][0] - expectedM00) > epsilon)
		return UNEXPECTED(eResult::failure, L"DirectX Perspective: M[0][0] incorrect");

	if (std::abs(proj[1][1] - expectedM11) > epsilon)
		return UNEXPECTED(eResult::failure, L"DirectX Perspective: M[1][1] incorrect");

	if (std::abs(proj[2][2] - expectedM22) > epsilon)
		return UNEXPECTED(eResult::failure, L"DirectX Perspective: M[2][2] incorrect");

	if (std::abs(proj[2][3] - 1.0f) > epsilon)
		return UNEXPECTED(eResult::failure, L"DirectX Perspective: M[2][3] should be 1.0");

	if (std::abs(proj[3][2] - expectedM32) > epsilon)
		return UNEXPECTED(eResult::failure, L"DirectX Perspective: M[3][2] incorrect");

	if (std::abs(proj[3][3] - 0.0f) > epsilon)
		return UNEXPECTED(eResult::failure, L"DirectX Perspective: M[3][3] should be 0.0");
	...
	if (std::abs(zNear - 0.0f) > epsilon)
		return UNEXPECTED(eResult::failure, L"DirectX Perspective: Near plane Z mapping incorrect");
	...
	if (std::abs(zFar - 1.0f) > epsilon)
		return UNEXPECTED(eResult::failure, L"DirectX Perspective: Far plane Z mapping incorrect");
	...
	const Vector3& up)
{
	...
	if (std::abs(right.length3() - 1.0f) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: Right vector not normalized");
	if (std::abs(upVec.length3() - 1.0f) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: Up vector not normalized");
	if (std::abs(forward.length3() - 1.0f) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: Forward vector not normalized");

	// Ортогональность
	if (std::abs(right.dot(upVec)) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: Right and Up vectors not orthogonal");
	if (std::abs(right.dot(forward)) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: Right and Forward vectors not orthogonal");
	if (std::abs(upVec.dot(forward)) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: Up and Forward vectors not orthogonal");
	...
	if (forwardDot < 0.999f)
		return UNEXPECTED(eResult::failure, L"LookAt: Forward vector direction incorrect");
	...
	if (std::abs(view[3][0] - expectedTx) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: Translation X component incorrect");
	if (std::abs(view[3][1] - expectedTy) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: Translation Y component incorrect");
	if (std::abs(view[3][2] - expectedTz) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: Translation Z component incorrect");
	if (std::abs(view[3][3] - 1.0f) > epsilon)
		return UNEXPECTED(eResult::failure, L"LookAt: M[3][3] should be 1.0");
