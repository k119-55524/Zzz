 ...
 * (Add -> Script / Game Script / Scene Script), либо ПКМ по пустому месту дерева (те же пункты
 * сразу в корне меню, без подменю Add - там же лежит Folder для папки), реализация -
 ...
 *   используется редактором для отслеживания переименований. В файлах конфигурации,
 *   которые создаются в будущем (сцены, префабы), ссылки на скрипты будут храниться как GUID.
 *   Это означает, что переименование скрипта не потребует правки ни одного файла сцены.
 ...
 *   GUID-ссылки из сцен в конкретные имена классов через таблицу `GUID → class_name` из `.meta`.
 ...
 *    всех `.hpp`/`.cpp` в `Assets/` с временем сборки `.editor/bin/scripts.dll`.
